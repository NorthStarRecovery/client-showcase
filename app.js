(() => {
  ...
  function renderProject(id) {
    ...
    const explainer = storyVisual && !isConceptual(project) ? `<section class="project-explainer reveal"><div class="section-heading"><div><span class="eyebrow mono">THE PROJECT, EXPLAINED</span><h2>${esc(storyVisual.title || 'Understanding the scope.')}</h2></div></div><figure><button data-photo="${esc(storyVisual.src)}" data-caption="${esc(storyVisual.caption || 'Conceptual project illustration.') }" aria-label="Enlarge conceptual project illustration"><img src="${esc(storyVisual.src)}" alt="${esc(storyVisual.caption || storyVisual.title || 'Conceptual project illustration')}" loading="lazy">${icon('maximize-2')}</button><figcaption><span class="mono">CONCEPTUAL ILLUSTRATION</span><p>${esc(storyVisual.caption || 'An illustration of the project scope. Not a project photograph.')}</p></figcaption></figure>${storyVisual.description ? `<p class="explainer-description">${esc(storyVisual.description)}</p>` : ''}</section>` : '';
    $('project-page').innerHTML = `<article class="project-experience">
      ...
      <section class="project-opening${isConceptual(project) ? ' conceptual-opening' : ''}"><div class="project-opening-photo">${photo(project,'','eager')}${isConceptual(project) ? '<span class="visual-label mono">CONCEPTUAL ILLUSTRATION / NOT PROJECT PHOTOGRAPHY</span>' : ''}</div><div class="project-opening-copy"><span class="eyebrow mono">${esc([project.location,project.period].filter(Boolean).join(' / '))}</span><h1 id="project-title" tabindex="-1">${esc(project.title)}</h1><p>${esc(project.outcome || project.summary)}</p><div class="project-actions">${selectedButton(project,'button inverse')}<button class="text-button" data-export-one="${esc(id)}">Download project ${icon('download')}</button></div></div></section>
  ...
  document.addEventListener('click', event => {
    ...
    if (button.dataset.photo && asset(button.dataset.photo)) {
      $('lightbox-image').src = button.dataset.photo; $('lightbox-image').alt = button.dataset.caption || byId.get(current)?.title || 'Project photograph'; $('lightbox-caption').textContent = $('lightbox-image').alt; showDialog($('lightbox')); return;
    }
