...
(function () {
  ...
  function conceptualCaption(caption) { return /\bconceptual illustration\b/i.test(caption) ? caption : 'Conceptual illustration' + (caption ? ' / ' + caption : ''); }
  ...
  function normalize(study, index) {
    ...
    const captions = {};
    imageValues.forEach(value => { const conceptual = study.visual && resourceUrl(study.visual.src) === resourceUrl(value) && study.visual.kind === 'conceptual'; const caption = text((study.imageCaptions || {})[value]) || (conceptual ? text(study.visual.caption) : ''); captions[resourceUrl(value)] = { caption: conceptual ? conceptualCaption(caption) : caption, conceptual: !!conceptual }; });
  ...
  function studyTemplate(study, images, logo, options, storyVisual) {
    ...
    const explanation = storyVisual ? '<div class="study-visual"><p class="eyebrow">Project explained</p><h3 class="visual-title">' + escape(storyVisual.title) + '</h3><figure class="explanation-scene"><img src="' + escape(storyVisual.src) + '" alt="' + (storyVisual.conceptual ? 'Conceptual illustration: ' : '') + escape(storyVisual.description || storyVisual.title) + '"><figcaption>' + escape(storyVisual.conceptual ? conceptualCaption(storyVisual.caption) : storyVisual.caption || 'Project illustration') + '</figcaption></figure>' + (storyVisual.description ? '<p class="visual-description">' + escape(storyVisual.description) + '</p>' : '') + '</div>' : '';
    return '<template class="study-template" data-id="' + study.id + '" data-title="' + escape(study.title) + '" data-sector="' + escape(study.sector) + '" data-restricted="' + study.restricted + '"><div class="opening-chrome">' + header(logo, study.sector || 'Project experience', study.restricted) + '<div class="project-heading"><h2 class="project-title' + (study.title.length > 65 ? ' long-title' : '') + '">' + escape(study.title) + '</h2><p class="project-meta">' + meta + '</p></div>' + (images[0] ? '<figure class="project-scene' + (images[0].conceptual ? ' conceptual-scene' : '') + '"><img src="' + escape(images[0].src) + '" alt="' + escape(images[0].caption || study.title) + '">' + (images[0].caption ? '<figcaption>' + escape(images[0].caption) + '</figcaption>' : '') + '</figure>' : '') + metrics + (lead ? '<p class="standfirst' + (lead.length > 290 ? ' long-summary' : '') + '">' + escape(lead) + '</p>' : '') + '</div><div class="narrative-blocks">' + body + '</div><div class="study-gallery">' + images.map(image => '<figure><img src="' + escape(image.src) + '" alt="' + escape(image.caption || study.title) + '">' + (image.caption ? '<figcaption>' + escape(image.caption) + '</figcaption>' : '') + '</figure>').join('') + '</div><div class="study-services">' + (study.services ? '<p class="service-line"><strong>Services</strong> / ' + escape(study.services) + '</p>' : '') + '</div>' + explanation + '</template>';
  }
  function runtime() {
    'use strict';
