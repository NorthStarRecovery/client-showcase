...
(function () {
  ...
  function runtime() {
    ...
    function fillColumns(columns, queue) {
      ...
      for (const [columnIndex, column] of columns.entries()) {
        ...
        // A paragraph may fail the minimum split length after its heading fits.
        const trailingHeading = column.lastElementChild;
        if (trailingHeading && trailingHeading.tagName === 'H3') {
          remaining.unshift({ tag: 'H3', className: trailingHeading.className, text: trailingHeading.textContent });
  ...
  async function stylesheet() {
    const response = await fetch(new URL('export.css?v=b14614926f83', base)); if (!response.ok) throw new Error('The portfolio design could not be loaded.');
