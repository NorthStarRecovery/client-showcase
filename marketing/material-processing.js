...
async function checkPdfObjects(bytes, context) {
  ...
  const { PDFDocument, PDFDict, PDFArray, PDFName, PDFStream } = await context.wait(import('../vendor/pdf-lib/pdf-lib.min.mjs?v=5dca6f2d0add'));
  let parsed;
  try {
...
async function preparePdf(bytes, context) {
  const pdfjs = await context.wait(import('../vendor/pdfjs/pdf.min.mjs?v=5dca6f2d0add'));
  context.check();
...
async function prepareImage(bytes, kind, context) {
  ...
  try {
    ...
    const { PDFDocument } = await context.wait(import('../vendor/pdf-lib/pdf-lib.min.mjs?v=5dca6f2d0add'));
