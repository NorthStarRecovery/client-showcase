...
async function checkPdfObjects(bytes, context) {
  ...
  const { PDFDocument, PDFDict, PDFArray, PDFName, PDFStream } = await context.wait(import('../vendor/pdf-lib/pdf-lib.min.mjs?v=f9576070ec66'));
  let parsed;
  try {
...
async function preparePdf(bytes, context) {
  const pdfjs = await context.wait(import('../vendor/pdfjs/pdf.min.mjs?v=f9576070ec66'));
  context.check();
...
async function prepareImage(bytes, kind, context) {
  ...
  try {
    ...
    const { PDFDocument } = await context.wait(import('../vendor/pdf-lib/pdf-lib.min.mjs?v=f9576070ec66'));
